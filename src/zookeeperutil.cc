...
void global_watcher(zhandle_t *zh, int type,
                        int status, const char *path, void *watcherCtx)
{
...
// zkclient启动连接zkserver  异步连接
// 启动ZooKeeper客户端，连接ZooKeeper服务器
void ZkClient::Start()
{
    // 从配置文件中读取ZooKeeper服ls务器的IP和端口
    ...
    */
    ...
    // 创建本地资源句柄成功，而不是连接zkserver成功，没收到响应(响应通过回调产生)
    ...
    // 等待连接成功
    std::unique_lock<std::mutex> lock(cv_mutex); // 为什么是unique_lock
